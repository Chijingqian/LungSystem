...
from layers import *
config = {}          # 配置项
config['anchors'] = [5., 10., 20.] #[ 10.0, 30.0, 60.]  # 配置anchor的大小
config['chanel'] = 1              #单通道 不像RGB图像一样3通道
config['crop_size'] = [96, 96, 96]  # 裁剪大小
config['stride'] = 4  # 没用到
config['max_stride'] = 16  # 没用到
config['num_neg'] = 800  # 没用到
config['th_neg'] = 0.02 # 没用到
config['th_pos_train'] = 0.5 # 没用到v # 训练阶段认为0.5以上就是正例
config['th_pos_val'] = 1  # 没用到
config['num_hard'] = 2  
...
config['augtype'] = {'flip':True,'swap':False,'scale':True,'rotate':False}        # 数据增强的类型
config['augtype'] = {'flip':True,'swap':False,'scale':True,'rotate':False}
config['blacklist'] = ['868b024d9fa388b7ddab12ec1c06af38','990fbe3f0a1b53878669967b9afd1441','adc3bbc63d40f8761c59be10f1e504c3']   # 黑名单
debug = True                   # 是否是DEBUG 模式 进行输出
...
class Bottleneck(nn.Module):
    def __init__(self, last_planes, in_planes, out_planes, dense_depth, stride, first_layer):
        ...
        if first_layer:
            self.shortcut = nn.Sequential(  # 顺序容器，参数可以是多个容器，或者是OrderedDict
    ...
    def forward(self, x):
        ...
        out = torch.cat([x[:,:d,:,:]+out[:,:d,:,:], x[:,d:,:,:], out[:,d:,:,:]], 1)  # torch.cat 连接操作
        if debug:
            print('bottleneck_5', out.size())
        out = F.relu(out)
        return out

class DPN(nn.Module):     # dpn模块
    def __init__(self, cfg):
        super(DPN, self).__init__()
        in_planes, out_planes = cfg['in_planes'], cfg['out_planes']  # TODO 看不懂
        ...
        self.linear = nn.Linear(out_planes[3]+(num_blocks[3]+1)*dense_depth[3], 2)#10)         # Applies a linear transformation to the incoming data XAT +B
