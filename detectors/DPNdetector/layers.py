...
class PostRes2d(nn.Module):        # 2D 的卷积Res 卷积神经网络结构
...
class PostRes(nn.Module):             # 3维的残差模块
    def __init__(self, n_in, n_out, stride = 1):
...
class Rec3(nn.Module):
    def __init__(self, n0, n1, n2, n3, p = 0.0, integrate = True): # integrate 求积分
    ...
    def forward(self, x0, x1, x2, x3):
        if self.p > 0 and self.training: # 训练阶段并且
            # torch.bernoulli 把概率变成随机分布
            coef = torch.bernoulli((1.0 - self.p) * torch.ones(8))  # coef = coefficient 系数
...
def hard_mining(neg_output, neg_labels, num_hard):
    """
    进行困难样本挖掘 意思是只选择一部分困难样本进行反向传播
    用于构建 topK loss
    """
    _, idcs = torch.topk(neg_output, min(num_hard, len(neg_output)))  # 取得topK 个数值
    neg_output = torch.index_select(neg_output, 0, idcs)  
    neg_labels = torch.index_select(neg_labels, 0, idcs)
    return neg_output, neg_labels

class Loss(nn.Module):                 # nn.module 可以进行拼接
    def __init__(self, num_hard = 0):
        super(Loss, self).__init__()
        self.sigmoid = nn.Sigmoid()
        self.classify_loss = nn.BCELoss()        # 二值交叉熵
        self.regress_loss = nn.SmoothL1Loss()   # 使用smoothl1损失函数
    ...
    def forward(self, output, labels, train = True):
        ...
        pos_idcs = pos_idcs.unsqueeze(1).expand(pos_idcs.size(0), 5)   # unsqueeze 解缩
        ...
        if len(pos_output)>0:  # 如果预测出了结节
            ...
            regress_losses_data = [l.item() for l in regress_losses]   # 由于版本的升级 由 l.data[0] 变成l.item()  # 每一个都计算回归损失
        ...
        classify_loss_data = classify_loss.item() # 更新
        ...
        return [loss, 
        ] + regress_losses_data + [pos_correct, pos_total, neg_correct, neg_total]
...
class GetPBB(object):
    ...
    def __init__(self, config):
        self.stride = config['stride']  # stride 步长
        self.anchors = np.asarray(config['anchors'])

    def __call__(self, output,thresh = -3, ismask=False):    # __call__ 即为可以想函数一样调用
        stride = self.stride
...
def nms(output, nms_th):   # 非极大抑制      nms_th为阈值
    if len(output) == 0:
        return output
...
def iou(box0, box1):
    ...
    intersection = overlap[0] * overlap[1] * overlap[2]  # 空间上的相交
    union = box0[3] * box0[3] * box0[3] + box1[3] * box1[3] * box1[3] - intersection  # 空间上的合并
    return intersection / union  # IOU

def acc(pbb, lbb, conf_th, nms_th, detect_th):
    """
    计算tp,fp,fn,lbb ，lbb 是啥啊
    """
...
def topkpbb(pbb,lbb,nms_th,detect_th,topk=30):
    ...
    fp = []
    tp = []
    while len(tp)+len(fp)<topk:  # 降低阈值，直到 fp +tp的值大于30，这样才能找到前30个
        conf_th = conf_th-0.2
